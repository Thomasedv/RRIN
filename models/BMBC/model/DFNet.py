...
class DynFilter(nn.Module):
    ...
    def forward(self, x, filter):
        ...
        for c in range(x.size(1)):
            x_localexpand.append(F.conv2d(x[:, c:c + 1, :, :], self.filter_localexpand,
                                          padding=self.padding))  # 5x5 kernel --> padding = 2
...
class RDB(nn.Module):
    def __init__(self, fusion_Feat, Conv_Num, Growth_rate):
        ...
        def LFF(intInput, intOutput):
            return torch.nn.Conv2d(in_channels=intInput, out_channels=intOutput, kernel_size=(1, 1), stride=(1, 1),
                                   padding=(0, 0))  # Conv_1x1 layer
        ...
        self.local_feature_fusion = LFF(fusion_Feat + Conv_Num * Growth_rate,
                                        fusion_Feat)  # G_0 : fusion_Feat, G : Growth_rate
